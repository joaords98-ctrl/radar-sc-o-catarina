...
export function StoryClusterCard({ group }: { group: StoryGroup }) {
  ...
  return (
    <article className="rounded-2xl border bg-white p-5 shadow-sm">
      ...
      <h3 className="mt-4 text-xl font-black leading-tight text-zinc-950">{group.headline}</h3>
      <p className="mt-2 text-sm text-zinc-500">
        {group.topic ?? 'Tema não classificado'} · {group.city ?? group.region ?? 'SC'} · atualizado {time}
      </p>

      <div className="mt-4 grid gap-3 rounded-xl bg-zinc-50 p-3 text-sm ring-1 ring-zinc-100 md:grid-cols-3">
        <div>
        ...
        <div>
          ...
          <p className="mt-1 font-semibold text-zinc-800">{formatLabels[group.suggestedFormat]}</p>
        </div>
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        <a className="rounded-xl bg-zinc-950 px-4 py-2 text-sm font-bold text-white" href={group.leadItem.link} target="_blank" rel="noreferrer">Abrir principal</a>
        <a className="rounded-xl bg-zinc-200 px-4 py-2 text-sm font-bold text-zinc-800" href={`/stories?focus=${encodeURIComponent(group.id)}`}>Ver grupo</a>
        <a className="rounded-xl bg-emerald-100 px-4 py-2 text-sm font-bold text-emerald-900" href={`/draft?newsId=${group.leadItem.id}`}>Gerar base</a>
      </div>
    </article>
  );
}
