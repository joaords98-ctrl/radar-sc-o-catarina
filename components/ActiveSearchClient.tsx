      ...
      <section className="rounded-2xl border bg-white p-5 shadow-sm">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {quickSearches.map((item) => (
            <button key={item} type="button" onClick={() => setQ(item)} className="rounded-full bg-zinc-100 px-3 py-2 text-xs font-bold text-zinc-700 hover:bg-zinc-200">
              {item}
            </button>
          ))}
        </div>
            ...
            <>
              <div className="flex flex-wrap items-start justify-between gap-4">
                <div>
                  ...
                  <h3 className="mt-1 text-2xl font-black">{result.items?.length ?? 0} notícias salvas no Radar</h3>
                ...
                <div className="flex flex-wrap gap-2">
                  {result.finalQuery ? <button onClick={() => copyText('consulta', result.finalQuery || '')} className="rounded-xl bg-zinc-100 px-4 py-2 text-sm font-bold text-zinc-800">Copiar consulta</button> : null}
                  {resultText ? <button onClick={() => copyText('resultados', resultText)} className="rounded-xl bg-emerald-100 px-4 py-2 text-sm font-bold text-emerald-900">Copiar resultados</button> : null}
                  <a href="/stories" className="rounded-xl bg-zinc-950 px-4 py-2 text-sm font-bold text-white">Ver pautas</a>
                </div>
              </div>
              ...
              <div className="mt-5 grid gap-3 lg:grid-cols-2">
                {result.items?.map((item) => (
                  <article key={item.link} className="rounded-xl border bg-zinc-50 p-4">
                    ...
                    <div className="mt-4 flex flex-wrap gap-2">
                      <a href={item.link} target="_blank" rel="noreferrer" className="rounded-xl bg-zinc-900 px-3 py-2 text-xs font-bold text-white">Abrir fonte</a>
                      {item.id ? <a href={`/draft?newsId=${item.id}`} className="rounded-xl bg-emerald-100 px-3 py-2 text-xs font-bold text-emerald-900">Gerar base</a> : null}
                    </div>
                  </article>
                ))}
              </div>
